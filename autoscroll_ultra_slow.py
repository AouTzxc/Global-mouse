...
class GlobalConfig:
    dead_zone = 20.0
    sensitivity = 2.0
    speed_factor = 1.0     # [调整] 默认值调低，方便开箱即用
    overlay_size = 60.0
...
class HelpDialog(QDialog):
    def __init__(self, parent=None):
        ...
        content.setText("""
            ...
            <p><b>⚡ 基础速度</b><br>全局倍率。<b>调低此值可实现极慢滚动。</b></p>
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Smooth Scroll XY (Ultra Slow)")
    ...
    def init_ui(self):
        ...
        # --- [升级] 支持自定义小数位数的 add_row 函数 ---
        def add_row(row_idx, label_text, val, min_v, max_v, callback, decimals=1):
        ...
        # 1. 加速度 (保持 1位小数)
        add_row(0, "加速度曲线", cfg.sensitivity, 1.0, 5.0, lambda v: setattr(cfg, 'sensitivity', v), decimals=1)
        
        # 2. [修改] 基础速度 (改为 2位小数，最小值 0.01)
        # 范围 0.01 - 5.00
        add_row(1, "基础速度", cfg.speed_factor, 0.01, 5.00, lambda v: setattr(cfg, 'speed_factor', v), decimals=2)
        
        # 3. 死区 (保持 1位小数)
        add_row(2, "中心死区", cfg.dead_zone, 0.0, 100.0, lambda v: setattr(cfg, 'dead_zone', v), decimals=1)
        
        # UI 大小 (整数逻辑单独处理或使用decimals=0)
        def update_size(val):
            cfg.overlay_size = val
            self.bridge.update_size.emit(int(val))
            self.bridge.preview_size.emit()
            
        # 这里单独手写UI大小的逻辑，或者复用add_row(decimals=0)
        # 为了简单，直接用上面的add_row，decimals=0就是整数
        add_row(3, "UI 大小", cfg.overlay_size, 30, 150, update_size, decimals=0)
            ...
            if cfg.active:
                try:
                    ...
                    if dist > cfg.dead_zone:
                        ...
                        # --- 核心修改 ---
                        # 0.00005 保持不变，但 cfg.speed_factor 现在可以非常小 (0.01)
                        # 当 speed_factor = 0.01, sens = 1.0 时
                        # 速度 = dist * 0.00005 * 0.01 = dist * 0.0000005
                        # 这比之前慢了 10-100 倍，足以满足超慢阅读需求
...
if __name__ == "__main__":
    myappid = 'adai.smoothscroll.xy.v2.2' 
