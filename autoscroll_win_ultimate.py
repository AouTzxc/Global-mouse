...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QGridLayout, QCheckBox, QSystemTrayIcon, QMenu, QAction, 
                             QMessageBox, QComboBox, QInputDialog, qApp) # [修复] 加上了 qApp
...
# --- 配置文件路径 ---
CONFIG_FILE = os.path.join(os.path.expanduser("~"), ".global_mouse_win_config.json")
...
class HelpDialog(QDialog):
    def __init__(self, parent=None):
        ...
        content.setText("""
            <p><b>🚀 后台运行</b><br>点击窗口右上角 X 不会退出，而是最小化到托盘。</p>
            <p><b>👻 启动隐藏</b><br>勾选“启动时最小化”，开机自启后将静默运行，不弹窗。</p>
            <p><b>💾 参数保存</b><br>可以保存多套配置方案。</p>
        """)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Global Mouse Ultimate")
    ...
    def init_system_tray(self):
        ...
        action_quit.triggered.connect(qApp.quit) # 现在这里可以正常工作了
    ...
    def closeEvent(self, event):
        if self.tray_icon.isVisible():
            self.hide()
            if not getattr(self, 'has_shown_msg', False):
                self.tray_icon.showMessage(
                    "已最小化到托盘",
                    "程序正在后台静默运行，双击图标可打开设置。",
    ...
    def init_ui(self):
        ...
        settings_panel.setStyleSheet("""
            ...
            QCheckBox { font-size: 14px; font-weight: 500; color: #000; spacing: 8px; }
        """)
        ...
        def add_row(key, row_idx, label_text, val, min_v, max_v, callback, decimals=1):
            ...
            spin.setSingleStep(step); spin.setFixedWidth(55)
        ...
        self.combo_presets.setStyleSheet("QComboBox { background: white; border-radius: 8px; padding: 5px; }")
        
        btn_save = QPushButton("保存")
        btn_save.setCursor(Qt.PointingHandCursor); btn_save.clicked.connect(self.save_new_preset)
        btn_save.setStyleSheet("QPushButton { background-color: #34C759; color: white; border-radius: 8px; padding: 6px 12px; }")
        
        btn_del = QPushButton("删除")
        btn_del.setCursor(Qt.PointingHandCursor); btn_del.clicked.connect(self.delete_preset)
        btn_del.setStyleSheet("QPushButton { background-color: #FF3B30; color: white; border-radius: 8px; padding: 6px 12px; }")
    ...
    def toggle_autorun(self, checked):
        if not self.autostart.set_autorun(checked):
            ...
            QMessageBox.warning(self, "权限不足", "设置开机自启失败，请以管理员身份运行。")
...
if __name__ == "__main__":
    myappid = 'adai.smoothscroll.win.ultimate' 
